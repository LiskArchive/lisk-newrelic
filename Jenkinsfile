...
pipeline {
		...
		stage('Run tests') {
			steps {
				ansiColor('xterm') {
					sh 'npm run test'
					sh '''
					cp ~/.coveralls.yml-lisk-newrelic .coveralls.yml
					npm run cover
					'''
				}
			}
	...
	post {
		success {
			...
			githubNotify context: 'continuous-integration/jenkins/lisk-newrelic', description: 'The build passed.', status: 'SUCCESS'
		}
		failure {
			script {
				build_info = get_build_info()
			...
			githubNotify context: 'continuous-integration/jenkins/lisk-newrelic', description: 'The build failed.', status: 'FAILURE'
		}
		aborted {
			githubNotify context: 'continuous-integration/jenkins/lisk-newrelic', description: 'The build was aborted.', status: 'ERROR'
		}
		always {
